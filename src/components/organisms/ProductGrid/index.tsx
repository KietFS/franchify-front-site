"use client";

import ProductCard from "@/components/molecules/ProductCard";
import useProducts from "@/hooks/useProducts";
import useStore from "@/hooks/useStore";
import React, { useEffect } from "react";

interface IProductGridProps {}

const ProductGrid: React.FC<IProductGridProps> = (props) => {
  const { getAllProducts, storeProdutcs, loading } = useProducts();
  const { currentStore } = useStore();

  useEffect(() => {
    currentStore && getAllProducts();
  }, [currentStore]);

  return (
    <>
      {loading ? (
        <div>
          <div className="bg-gray-200 animate-pulse w-1/2 h-[60px] mb-8"></div>
          <div className="w-full  grid grid-cols-4 gap-4">
            {Array(16)
              .fill(1)
              ?.map((item, index) => (
                <div className="bg-gray-200 animate-pulse w-full h-[360px]"></div>
              ))}
          </div>
        </div>
      ) : (
        <>
          {storeProdutcs?.length > 0 && (
            <div>
              <h1 className="text-4xl font-bold text-gray-600 mb-16">
                Sản phẩm nổi bật
              </h1>
              <div className="w-full grid grid-cols-4 gap-y-4 gap-x-4">
                {storeProdutcs?.map((item: any, index: number) => (
                  <ProductCard
                    handleItemClick={() => {}}
                    item={item}
                    index={index}
                  />
                ))}
              </div>
            </div>
          )}
        </>
      )}
    </>
  );
};

export default ProductGrid;
